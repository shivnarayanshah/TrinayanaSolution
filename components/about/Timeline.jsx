...
export default function Timeline() {
    ...
    return (
        <div className="timeline-container relative max-w-4xl mx-auto py-20 px-6">
            <div
                ref={lineRef}
                className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-1 bg-accent origin-top hidden md:block"
            />
            ...
            {timelineData.map((item, i) => (
                ...
                >
                    {/* Year Circle */}
                    <div className="absolute left-1/2 -translate-x-1/2 w-14 h-14 bg-primary border-4 border-creamy-milk rounded-full z-10 hidden md:flex items-center justify-center text-creamy-milk text-sm font-black shadow-xl">
                        {item.year}
                    </div>

                    <div className={`md:w-1/2 ${i % 2 === 0 ? "md:pl-16 text-right md:text-left" : "md:pr-16"} text-center md:text-left`}>
                        <div className={`inline-block px-4 py-1 bg-accent text-primary text-xs font-black uppercase tracking-tighter mb-4 rounded-full md:hidden`}>
                            {item.year}
                        </div>
                        <h4 className="text-3xl font-black text-primary uppercase mb-3 leading-none">{item.title}</h4>
                        <p className="text-primary/70 font-medium leading-relaxed">{item.description}</p>
                    </div>
